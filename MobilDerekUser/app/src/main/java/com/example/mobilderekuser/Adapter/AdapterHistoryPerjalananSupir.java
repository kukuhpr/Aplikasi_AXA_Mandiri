...
public class AdapterHistoryPerjalananSupir extends RecyclerView.Adapter<AdapterHistoryPerjalananSupir.HolderHistory> {

    private Context context;
    private ArrayList<TripPlanModel> tripPlanList;

    public AdapterHistoryPerjalananSupir(Context context, ArrayList<TripPlanModel> tripPlanList) {
    ...
    @NonNull
    @Override
    public HolderHistory onCreateViewHolder(@NonNull @NotNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(context).inflate(R.layout.row_order_history, parent,false);
        return new AdapterHistoryPerjalananSupir.HolderHistory(view);
    }

    @Override
    public void onBindViewHolder(@NonNull @NotNull AdapterHistoryPerjalananSupir.HolderHistory holder, int position) {
//get Data
        TripPlanModel tripPlanModel = tripPlanList.get(position);

        String orderId = tripPlanModel.getTripId();
        String nama_supir = tripPlanModel.getUserModel().getNamaUser();
        String nama_usaha = tripPlanModel.getUserModel().getNoHpUser();
        ...
        String foto_user = tripPlanModel.getUserModel().getProfileImage();
